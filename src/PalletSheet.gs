...
function lookupPalletById_(palletId) {
  ...
  for (let r = 1; r < data.length; r++) {
    ...
    return {
      ...
      ExclusionStatus:    idx['ExclusionStatus'] !== undefined ? String(data[r][idx['ExclusionStatus']] || '').trim() : '',
      ExclusionReason:    idx['ExclusionReason'] !== undefined ? String(data[r][idx['ExclusionReason']] || '').trim() : '',
      ExcludedAt:         idx['ExcludedAt']      !== undefined ? (data[r][idx['ExcludedAt']] || null)                : null,
      ExcludedBy:         idx['ExcludedBy']      !== undefined ? String(data[r][idx['ExcludedBy']]      || '').trim() : '',

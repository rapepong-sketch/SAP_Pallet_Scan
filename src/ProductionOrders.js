...
// ============================================================================
// DIAGNOSTIC — why operations are missing for a specific order (read-only)
// ============================================================================

/**
 * DIAGNOSTIC ONLY — does not touch fetchOperationsForMO_ / getOperationsForOrder
 * or any existing routing logic. Investigates why the routing query returns 0
 * operations for an order that has them in SAP Fiori (e.g. 1000036350, ops
 * 0010/0020/0030, PDSM, Plant 1100).
 *
 * Runs three independent, read-only probes against API_PRODUCTION_ORDER_2_SRV
 * and logs the raw HTTP status + body for each, so we can tell whether the
 * cause is:
 *   (A vs B) the OrderID/ManufacturingOrder key format (raw vs 12-digit padded)
 *   (A/B vs C) the entityset-direct path vs the header + $expand path
 *   (filter)  a $filter clause silently excluding rows
 *
 * Run from the Apps Script editor (Run ▶ debugOrderRouting) and read the
 * Logger output (View → Logs / Executions).
 */
function debugOrderRouting() {
  const TEST_ORDER = '1000036350'; // ⚠️ change this to test a different order
...
/**
 * PROBE A/B helper — queries the ProductionOrderOperation entityset directly
 * (NOT via header $expand) filtered by ManufacturingOrder + Plant. Used by
 * debugOrderRouting() to test whether the OrderID key format changes the
 * result set. Read-only; muteHttpExceptions so non-2xx responses can be
 * inspected instead of thrown.
 * @param {string} orderId - raw or 12-digit padded ManufacturingOrder key
 * @param {string} label - Logger.log prefix only, e.g. 'A (raw key)'
 * @return {number} operation count returned, or -1 on non-2xx HTTP
 */
...
/**
 * PROBE C helper — header entity (A_ProductionOrder_2) with
 * $expand=to_ProductionOrderOperation, i.e. the same navigation path
 * fetchOperationsForMO_ uses, but with NO $select so the full raw operation
 * rows are visible. Used by debugOrderRouting() to test entityset-direct vs
 * expand-via-header. Read-only; muteHttpExceptions so non-2xx responses can
 * be inspected instead of thrown.
 * @param {string} orderId - raw or 12-digit padded ManufacturingOrder key
 * @param {string} label - Logger.log prefix only, e.g. 'C (raw key)'
 * @return {number} operation count under the expand, or -1 on non-2xx HTTP
 */
...
/**
 * DIAGNOSTIC ONLY — confirms whether the "missing operations" symptom for a
 * specific order is a stale/empty ProductionOrders sheet cache rather than a
 * live-fetch bug. Read-only: no sheet writes, no SAP writes (logEvent only).
 *
 * Inspects the ProductionOrders sheet row for TEST_ORDER (read by column
 * name), logs the raw OperationsJSON / Operations / FinalOperation cell
 * values and how many ops parse out of OperationsJSON, then calls
 * getOperationsForOrder() — the same cache→fetch fallback path the mobile
 * scanner / PalletSheet use — and logs how many ops it returns and whether
 * the sheet cache (getOpsForMo_) already had them or it fell back to a live
 * SAP fetch (fetchOperationsForMO_).
 *
 * Run from the Apps Script editor (Run ▶ inspectOrderCache) and read the
 * Logger output (View → Logs / Executions).
 */
function inspectOrderCache() {
  const TEST_ORDER = '1000036350'; // ⚠️ change this to inspect a different order
  ...
  // Check the cache layer directly (getOpsForMo_) so we can tell whether
  // getOperationsForOrder()'s result below came from the sheet cache or fell
  // back to a live SAP fetch (fetchOperationsForMO_), without touching WebApp.gs.
...
/**
 * Manual test entry point for refreshOrderOperationCache() — run from the
 * Apps Script editor. Confirms the backfill on ONE known-stale order
 * (1000036350 — see inspectOrderCache() diagnostic) before any bulk pass.
 */
